 ...
 * cached locks.
 */
static long ldlm_pools_count(ldlm_side_t client, unsigned int gfp_mask)
{
	long total = 0, nr_ns;
	struct ldlm_namespace *ns;
	void *cookie;
...
static long ldlm_pools_scan(ldlm_side_t client, int nr, unsigned int gfp_mask)
{
	long freed = 0;
        int tmp, nr_ns;
	struct ldlm_namespace *ns;
	...
	return (client == LDLM_NAMESPACE_SERVER) ? -1 : freed;
}

static long ldlm_pools_srv_count(struct shrinker *s, struct shrink_control *sc)
{
	return ldlm_pools_count(LDLM_NAMESPACE_SERVER, sc->gfp_mask);
}

static long ldlm_pools_srv_scan(struct shrinker *s, struct shrink_control *sc)
{
	return ldlm_pools_scan(LDLM_NAMESPACE_SERVER, sc->nr_to_scan,
			       sc->gfp_mask);
}

static long ldlm_pools_cli_count(struct shrinker *s, struct shrink_control *sc)
{
	return ldlm_pools_count(LDLM_NAMESPACE_CLIENT, sc->gfp_mask);
}

static long ldlm_pools_cli_scan(struct shrinker *s, struct shrink_control *sc)
{
