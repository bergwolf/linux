...
int cfs_hash_debug_str(cfs_hash_t *hs, struct seq_file *m)
{
	...
	seq_printf(m, "%-*s ", CFS_HASH_BIGNAME_LEN, hs->hs_name);
	seq_printf(m, "%5d ",  1 << hs->hs_cur_bits);
	seq_printf(m, "%5d ",  1 << hs->hs_min_bits);
	seq_printf(m, "%5d ",  1 << hs->hs_max_bits);
	seq_printf(m, "%d.%03d ", __cfs_hash_theta_int(theta),
		      __cfs_hash_theta_frac(theta));
	seq_printf(m, "%d.%03d ", __cfs_hash_theta_int(hs->hs_min_theta),
		      __cfs_hash_theta_frac(hs->hs_min_theta));
	seq_printf(m, "%d.%03d ", __cfs_hash_theta_int(hs->hs_max_theta),
		      __cfs_hash_theta_frac(hs->hs_max_theta));
	seq_printf(m, " 0x%02x ", hs->hs_flags);
	seq_printf(m, "%6d ", hs->hs_rehash_count);
	...
	seq_printf(m, "%7d ", total);
	seq_printf(m, "%7d ", maxdep);
	seq_printf(m, "%7d ", maxdepb);
	for (i = 0; i < 8; i++)
