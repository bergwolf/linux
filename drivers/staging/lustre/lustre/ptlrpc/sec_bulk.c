 ...
 */
static long enc_pools_shrink_count(struct shrinker *s, struct shrink_control *sc)
{
	/*
 ...
 * we try to keep at least PTLRPC_MAX_BRW_PAGES pages in the pool.
 */
static long enc_pools_shrink_scan(struct shrinker *s, struct shrink_control *sc)
{
	spin_lock(&page_pools.epp_lock);
	sc->nr_to_scan = min_t(long, sc->nr_to_scan,
